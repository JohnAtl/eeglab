...
if nargin < 3
    ...
    if typecomp
        inistr       = { ['1:' int2str(length(EEG.chanlocs))] ['''freqrange'', [2 ' num2str(min(50, EEG.srate/2)) ']'] '' 1};
    else
        inistr       = { ['1:' int2str(size(EEG.icawinv, 2))] ['''freqrange'', [2 ' num2str(min(50, EEG.srate/2)) ']'] '' 1};
    end
    ...
    if length(chanorcomp) > PLOTPERFIG*2 % JTJ
...
%		set( button, 'callback', {@pop_prop_extended, EEG, typecomp, ri, NaN, spec_opt, erp_opt, scroll_event, classifier_name} );
		set( button, 'callback', {@pop_prop_extended, EEG, typecomp, ri, gcbo, spec_opt, erp_opt, scroll_event, classifier_name} );
	end;
    if typecomp
