...
if nargin < 3
    ...
    if typecomp
        inistr       = { ['1:' int2str(length(EEG.chanlocs))] ['''freqrange'', [1 ' num2str(min(50, EEG.srate/2)) ']'] '' 1};
    else
        inistr       = { ['1:' int2str(size(EEG.icawinv, 2))] ['''freqrange'', [1 ' num2str(min(50, EEG.srate/2)) ']'] '' 1};
    end
    ...
    if length(chanorcomp) > PLOTPERFIG*2
...
for ri = chanorcomp
    ...
    if typecomp
        set( button, 'backgroundcolor', eval(fastif(EEG.reject.gcompreject(ri), COLREJ,COLACC)), 'string', EEG.chanlocs(ri).labels); 	
    else
